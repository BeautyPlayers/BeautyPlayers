...
<!-- <nav class="nav-sections " id="p">-->
<!--  <ul class="menu">-->
     
<!--@foreach ($categories as $key =>$v)-->
<!--      @foreach ($v->subCategories as $k => $vv)-->
<!--    <li class="menu-item"><a class="menu-item-link active" href="p{{ $vv->id }}">{{ $vv->name }}--- bbb {{ $vv->id }}</a></li>-->
<!--      @endforeach-->
<!--        @endforeach-->
<!--     <div class="active-line"></div>-->
<!--  </ul>-->
...
position: relative;top: -25px;width:100%;">
  <!-- <section class="page-section">
    <h2 class="section-title" id="about">About</h2> -->
      ...
      @foreach($producstList as $a=>$b)
      @foreach($b->products as $k=>$v)
      <section class="page-section ">
        <div class="container">
            @if(!empty($v->main_category))
          <h2 class="section-title" id="p{{ $b->id }}" style="background: silver; font-size: 20px;">{{ $v->main_category }}</h2> 
            @endif

    <!--<h2 class="section-title" id="p{{ $v->id }}" style="background: silver; font-size: 20px;">{{ $v->category->name }}</h2> -->
          <div class="row mb-3">
            <div class="col-md-12">
              <div class="d-flex flex-row border rounded">
                  ...
                  <img src="{{ uploaded_asset($v->thumbnail_img) }}" class="img-thumbnail border-0" style="height: 149px;width: 169px;"/>
